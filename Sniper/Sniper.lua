...
state.optimal_d_pos = function(c)
    ...
    local cast_pos = Vector(mp.x + dx / d * g, mp.y + dy / d * g, mp.z or 0)

    -- v6.15.248: danger-aware adjustment for combo D, mirroring
    -- auto_grenade_tick's v6.15.246 (B) rotation block. Combo D fires the
    -- grenade at the attacker; when Sniper is within the 375u self-push
    -- radius of cast_pos (close-range combos -- starter_dr against a melee
    -- diver), Sniper gets pushed by his own grenade. The baseline cast
    -- direction is straight toward the attacker, so Sniper's push direction
    -- is straight away -- but if other enemies sit on that "away" side, the
    -- push lands him in a worse spot than where he stood. The shared helper
    -- picks the lowest-danger landing among 7 rotations, then this routine
    -- rotates cast_pos around Sniper to match. The attacker-in-blast
    -- geometric constraint (pred - rotated_cast):Length2D() <= 375 keeps
    -- the rotated cast viable for the offensive intent (D still hits the
    -- attacker); if no rotation passes the constraint, the original
    -- straight-toward cast_pos stands. 0deg ties go to the baseline via
    -- strict less-than (consistent with v6.15.245 helper), so a 1v1 combo
    -- against a single attacker keeps the historical geometry.
    local GRENADE_RADIUS  = 375
    local sniper_to_cast  = mp:Distance2D(cast_pos)
    if sniper_to_cast > 1 and sniper_to_cast <= GRENADE_RADIUS and c and c.target then
        local diff = cast_pos - mp
        local L = math.sqrt(diff.x * diff.x + diff.y * diff.y)
        if L > 1 then
            local toward = Vector(diff.x / L, diff.y / L, 0)
            local SNIPER_PUSH = 475
            local escape_dir = state.pick_escape_dir(mp, toward, SNIPER_PUSH, c.target)
            if escape_dir then
                local cand = mp - escape_dir * sniper_to_cast
                if (tp - cand):Length2D() <= GRENADE_RADIUS then
                    if math.abs(cand.x - cast_pos.x) > 1
                       or math.abs(cand.y - cast_pos.y) > 1 then
                        tlog(2, "optimal_d_pos_rotated", {
                            land_x = string.format("%.0f", mp.x + escape_dir.x * SNIPER_PUSH),
                            land_y = string.format("%.0f", mp.y + escape_dir.y * SNIPER_PUSH),
...
local SAVE_FIRE = {
    ...
    grenade_at_caster   = {
        ...
        fire  = function(intent, threat_caster)
            ...
            local GRENADE_RADIUS = 375
            -- v6.15.253: hoisted from below for use by the facing_ok closure
            -- in the rotation block. The post-pick gate at line below still
            -- runs as a defence-in-depth backstop.
            local MAX_TURN_FOR_GRENADE_AT_CASTER = 120
            local sniper_to_pred_d = me_pos:Distance2D(caster_pred)
            local cx, cy, midpoint_used, rotated_used
            ...
            -- v6.15.253: midpoint baseline + facing-aware rotation. v6.15.252
            -- log showed grenade_at_caster_skip_facing four times when Sniper
            -- was back-turned to PA (angle 131-180deg vs the 120deg gate) --
            -- chain fell through to pike, pike then failed during PA's blink
            -- (cast_verify_double_fail). Symptom user reported: "If sniper not
            -- facing PA direction it might fire on awkward position". Fix
            -- mirrors v6.15.247's grenade_self_cast_point pattern: pass a
            -- facing predicate INTO state.pick_escape_dir so the helper only
            -- considers angles whose corresponding cast_point Sniper can reach
            -- within the 120deg turn budget. The escape_dir reshapes cast_pos
            -- via cast = me_pos - escape_dir * (dist/2), then the caster is
            -- verified still within blast radius (rotation breaks geometrically
            -- at dist > ~335u; the helper return-nils for those). Falls back
            -- to baseline midpoint when no rotation is found.
            if midpoint_used and sniper_to_pred_d > 1 then
                local toward = Vector((caster_pred.x - me_pos.x) / sniper_to_pred_d,
                                      (caster_pred.y - me_pos.y) / sniper_to_pred_d, 0)
                local offset_rot = sniper_to_pred_d / 2
                local function facing_ok(esc_dir, _landing)
                    local cx2 = me_pos.x - esc_dir.x * offset_rot
                    local cy2 = me_pos.y - esc_dir.y * offset_rot
                    local cand_pt = Vector(cx2, cy2, caster_pos.z)
                    local ang = math.deg(math.abs(NPC.FindRotationAngle(me, cand_pt)))
                    return ang <= MAX_TURN_FOR_GRENADE_AT_CASTER
                end
                local escape_dir = state.pick_escape_dir(me_pos, toward, 475,
                                                          threat_caster, facing_ok)
                if escape_dir then
                    local cand_x = me_pos.x - escape_dir.x * offset_rot
                    local cand_y = me_pos.y - escape_dir.y * offset_rot
                    local dxc = caster_pred.x - cand_x
                    local dyc = caster_pred.y - cand_y
                    if math.sqrt(dxc * dxc + dyc * dyc) <= GRENADE_RADIUS then
                        cx, cy = cand_x, cand_y
                        rotated_used = true
                    end
                end
            ...
            -- v6.15.253: MAX_TURN_FOR_GRENADE_AT_CASTER hoisted above to the
            -- rotation block's facing_ok closure. The post-pick gate below
            -- still runs as a backstop -- if the helper somehow picked an
            -- angle that fails facing (filter_fn race / nil return path),
            -- the gate refuses and the chain falls through as before.
            ...
            tlog(3, "grenade_at_caster_cast_plan", {
                ...
                rotated = rotated_used and "y" or "n",
            })
