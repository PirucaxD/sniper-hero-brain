...
local function blink_escape_position(threat_caster_hint)
    ...
    -- v6.15.240 (clue C4): try a few escape angles away from the threat and
    -- keep the lowest-danger landing that still passes SafePushDestination,
    -- so a blink escaping the primary threat does not drop Sniper next to a
    -- backliner. 0deg (straight away) is always a candidate, so this never
    -- does worse than the old single-point behaviour.
    local best_dest, best_danger
    for _, deg in ipairs({ 0, -35, 35, -65, 65 }) do
        local rad   = math.rad(deg)
        local c, s  = math.cos(rad), math.sin(rad)
        local dir   = Vector(toward_threat.x * c - toward_threat.y * s,
                             toward_threat.x * s + toward_threat.y * c, 0)
        local cand  = me_pos - dir * 1180
        if SafePushDestination(cand, threat_caster_hint) then
            local dng = state.danger_at_pos(cand)
            if not best_danger or dng < best_danger then
                best_danger, best_dest = dng, cand
            end
        end
    end
    return best_dest
...
local function save_channel_on_self(caster, threat_mod)
    ...
    local pos = NPCLib.origin(caster)
    if pos and issue_cast_position("channel_on_me_d", ability(A.D), pos) then
