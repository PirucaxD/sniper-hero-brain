...
local Timing = require("lib.timing")    -- v6.15 A5
...
local cluster_around_target     -- v6.14.1 C1: build_layer1_ctx calls this in
                                -- its IIFE, before cluster_around_target's own
                                -- local-function declaration further down the
                                -- file. Without the forward decl the reference
                                -- would resolve to a nil global at runtime.
local take_aim_range_bonus      -- v6.15.79 (LIQUIPEDIA_REF.md Take Aim levels):
                                ...
                                -- Same pattern as cluster_around_target —
                                -- closures resolve to THIS local at call time.
local self_take_aim_state       -- v6.15.81: forward-decl for the same reason.
                                -- build_layer1_ctx exposes c.self_take_aim_active
                                -- and c.self_ms_slow_pct from this helper.
...
state.R_PHASE_START_DEADLINE = 0.6 -- v6.15.141, restored v6.15.218: if R has not entered its cast phase this many seconds after dispatch it was cancelled on order receipt; clear the R markers and downgrade the throttle so starter_tick / teamfight_tick re-appraise within ~1s. (v6.15.217 raised this to 1.0 for a per-tick retry that v6.15.218 removed.)
state.r_retry_count          = 0   -- v6.15.226: bounded retries used on the current R attempt-chain; reset on R success / give-up in r_abort_tick.
state.R_RETRY_MAX            = 2   -- v6.15.226: max retries when R is cancelled in its pre-phase by the native order flood (the OnPrepareUnitOrders veto cannot see native orders).
...
state.fight_start_t     = 0    -- v6.15.52 (G4): first-enemy-in-scan timestamp. 0 = not in fight.
...
state.kinetic_fields    = {}      -- v6.15.58 (G12): index → { thinker, mod_name, caster }
                                ...
                                -- Updated by recompute_candidates: set when first enemy enters
                                -- scan radius, cleared when no enemies present. Drives the
                                -- "fight has been live > 2s" gate that refuses R commits on
                                -- full-HP teamfight targets before the fight has developed.
state.last_q_t       = 0
state.last_e_t       = 0
state.last_d_t       = 0
state.last_r_t       = 0
...
local function blink_escape_position(threat_caster_hint)
    ...
    local dest = me_pos - toward_threat * 1180
    if not SafePushDestination(dest, threat_caster_hint) then return nil end
    return dest
end
...
local function grenade_self_cast_point(threat_caster_hint)
    ...
    local cast_point  = me_pos + toward_threat * cast_offset

    -- Facing-angle gate. The cast point is BETWEEN Sniper and the threat;
    -- if Sniper is currently facing AWAY from the threat (kiting, attacking
    -- a different target), the cast point is behind Sniper. The engine
    -- queues the grenade order but Sniper has to TURN to face the cast
    -- point before the cast animation can play. At Sniper's 0.7 turn rate,
    -- a 180° turn takes ~0.45s — longer than Pudge Dismember's 0.3s cast
    -- point window. The grenade lands AFTER Sniper is already stunned.
    --
    -- 90° threshold: Sniper's natural facing must be within 90° of the
    -- cast point. Beyond that, fall through to the next save in the chain
    -- (typically Pike-on-enemy, which has no facing dependency in v4.8+).
    -- For pre_cast and at_impact saves where the threat is about to hit,
    -- the chain proceeding to Pike is the right call. For armed_threat
    -- saves with longer ETA windows, Sniper has time to turn but the cost
    -- is small — Pike-on-enemy is still a clean save.
    -- v6.15.45 dropped this gate entirely. v6.15.47 puts it back at 120°
    -- after the user observed: 'For pudge ult when facing the other
    -- direction, granade lands late.' Log evidence: grenade_at_caster
    -- dispatched on Pudge Dismember twice with Sniper facing away;
    -- cast_verify showed fired=n on both attempts (age_ms=1533) — Sniper
    -- got stunned mid-turn (Dismember 0.3s cast lands before Sniper finishes
    -- the 0.45s rotation). 120° threshold = ~0.3s of turn time at Sniper's
    -- 0.6 turn rate. Within that window the grenade lands inside Dismember's
    -- cast point and breaks the channel. Beyond it, refuse → chain falls to
    -- Pike (item-cast, no facing requirement, fires cleanly even when
    -- Sniper is back-turned to the threat). Pike then dispatches the same
    -- save in 0.5s cast — slightly slower but ACTUALLY fires, which beats
    -- the never-firing grenade. Threshold tuned for Pudge Dismember (0.3s
    -- cast); longer-cast channels (Bane grip 0.6s) tolerate more turn but
    -- are also fine falling to Pike when facing way off.
    local angle_to_cast = math.abs(NPC.FindRotationAngle(me, cast_point))
    local MAX_TURN_FOR_GRENADE_SELF = 120
    if angle_to_cast > MAX_TURN_FOR_GRENADE_SELF then
        tlog(3, "grenade_self_skip_facing", {
            angle = string.format("%.0f", angle_to_cast),
            max = MAX_TURN_FOR_GRENADE_SELF,
        })
        return nil
    end

    -- Predicted landing: opposite direction from cast point. Pass the
    -- threat caster through so SafePushDestination prioritizes "distance
    -- from THIS threat" over the generic centroid check — that's the
    -- whole point of an anti-gap-close save in a multi-enemy scenario.
    local end_pos = me_pos - toward_threat * push_distance
    if not SafePushDestination(end_pos, threat_caster_hint) then
        tlog(3, "grenade_self_skip_unsafe_dest", {
            end_x = string.format("%.0f", end_pos.x or 0),
            end_y = string.format("%.0f", end_pos.y or 0),
        })
        return nil
    end
...
-- v6.15.199 (audit C7): API-first channel detection — same shape as
-- v6.15.197 B2 applied to build_layer1_ctx.target_channeling.
...
local function ScoreUltTarget(target)
    ...
    if Target.IsKillable(target) then
        ...
        -- already included inside `rc_attack_damage_with_procs` (line 2110),
    ...
    --   >3.0s → full veto (line ~924, unchanged).
    if fog_age > 1.0 then
    ...
    -- branch at line ~935) outranks the role adjustment — we DON'T refuse
...
SafePushDestination = function(dest_pos, threat_caster_hint)
    ...
    -- Centroid fallback: no specific threat known, use the rough centroid
    -- check to avoid pushing INTO the enemy team.
    local enemies = NPCs.InRadius(me_pos, 1500, Entity.GetTeamNum(me),
        Enum.TeamType.TEAM_ENEMY, true, true)
    if enemies and #enemies > 0 then
        local centroid = VectorCenter(enemies)
        if centroid then
            local d_now  = me_pos:DistanceSqr2D(centroid)
            local d_dest = dest_pos:DistanceSqr2D(centroid)
...
local function recompute_candidates()
    ...
    -- v6.15.52 (G4): fight_start_t tracks when the first enemy entered our
    -- scan radius. Drives the "fight has been live > 2s" gate in combo
    -- commit_preds — refuses R on full-HP targets before the fight has
    -- developed (initiation is your teammate's job; Sniper's R is cleanup).
    -- Cleared when no enemies in range so the next engagement starts fresh.
    if list and #list > 0 then
        if (state.fight_start_t or 0) == 0 then
            state.fight_start_t = now()
        end
    else
        state.fight_start_t = 0
    end
...
-- code's intent at keen_scope_bonus + line 2110). It belongs in
...
r_kill_prediction = function(target, take_aim_active)
    ...
    -- inside r_physical via rc_attack_damage_with_procs (line 2110). The
...
-- Returns 0..3 if R+N*Q+RC kills (use N stacks), or 4 if even 3Q isn't enough
-- (combo not viable — too tanky). Used by snipe_e_r to decide whether to add
-- Q2 / Q3 stacking steps and to gate commit when no other kill path exists.
local function compute_q_kill_floor(eff_hp, r_dmg, shrap_per_q, rc_2s)
    if eff_hp <= r_dmg + rc_2s then return 0 end                            -- R + RC alone
    if eff_hp <= r_dmg + rc_2s + shrap_per_q then return 1 end              -- + 1Q
    if eff_hp <= r_dmg + rc_2s + shrap_per_q * 2 then return 2 end          -- + 2Q stacked
    if eff_hp <= r_dmg + rc_2s + shrap_per_q * 3 then return 3 end          -- + 3Q stacked
    return 4                                                                -- not viable
end
...
-- v6.15.43 (fix): moved OFFENSIVE_KITE_HERO_OVERRIDES above build_layer1_ctx.
-- v6.15.42 declared it later in the file, but build_layer1_ctx
-- references it via c.kite_prefers_pike.
-- Lua compiles closures with lexical scope determined at definition time,
-- so a forward reference to a later-declared local is read as a global —
-- which is nil under UCZone's sandbox (_G isn't exposed). v6.15.42 demo
-- produced 546 Lua errors at line 1838: "attempt to index a nil value
-- (global 'OFFENSIVE_KITE_HERO_OVERRIDES')". Brain was crashing every
-- candidate evaluation → 0 combos dispatched → PA fix never exercised.
-- The table must be declared BEFORE the first function that reads it.
local OFFENSIVE_KITE_HERO_OVERRIDES = {
    npc_dota_hero_phantom_assassin = "pike",
}
...
local function build_layer1_ctx(target, score_ult)
    ...
    -- v6.15.198 (audit C3): cache the two enemy active-heal modifier
    -- checks. The same `modifier_item_satanic_unholy_rage` and
    -- `modifier_item_bloodthorn_active` queries are read twice in
    -- build_layer1_ctx — once here (eff_hp_magical bump) and once in
    -- the `target_active_heal` field below. Without the cache each
    -- ctx build did 4 `NPC.HasModifier` engine calls; build_layer1_ctx
    -- is called up to 3× per teamfight tick → 12 calls/tick saved to
    -- 6. Behaviour-equivalent — engine state cannot change between
    -- same-frame calls on the same target.
    ...
    local RC_MIN_DAMAGE_FACTOR = 0.5
    local rc_2s = rc_damage_over(2, true, d) * RC_MIN_DAMAGE_FACTOR
    local setup_killable = (eff_hp_magical <= (r_dmg_at_d + rc_2s))
                       and (d <= (atk_range + take_aim_range_bonus()))
    -- v6.9.3: how many stacked Q does it take to close the kill?
    -- 0 = R+RC alone, 1-3 = need that many Q stacks, 4 = not viable.
    local shrap_per_q = shrap_damage_per_q_effective()
    local q_kill_floor = compute_q_kill_floor(eff_hp_magical, r_dmg_at_d, shrap_per_q, rc_2s)
    ...
    return {
        ...
        d                 = d,
        score_ult         = score_ult,
        -- v6.15.201 (audit D8): nil-guard t_pos. GetAbsOrigin can return
        -- nil on a target that died mid-tick between Target.IsAlive (passed
        -- above) and this read. False is the safe default (target out of
        -- cone → no cone-gated bonuses).
        in_cone           = (t_pos and math.abs(NPC.FindRotationAngle(me, t_pos)) <= 70) or false,
        mana              = mana,
        ...
        target_channeling = (function()
            -- v6.15.197 (audit B2): API-based channel detection. The
            -- v6.15.189 idiom NPC.GetChannellingAbility(target) returns
            -- the channelled ability handle if the target is currently
            -- channelling — modifier-name-free, so robust against the
            -- catalog rot that motivated v6.15.189 in the first place
            -- (Pudge Dismember's _pull suffix etc.). The legacy
            -- ENEMY_CHANNEL_MODIFIERS iteration is preserved as a
            -- fallback for cases the live API may miss (engine quirks);
            -- MODIFIER_STATE_CHANNELED is the last fallback. The other
            -- catalog consumers (lines 1068 ScoreUltTarget, 8421 OnModifier
            -- threat-route) are LEFT ALONE — they need the modifier name
            -- specifically (for the TP +50 bonus and threat-catalog
            -- dispatch respectively).
            if NPC.GetChannellingAbility
               and NPC.GetChannellingAbility(target) then
                return true
            end
            for mod, _ in pairs(ENEMY_CHANNEL_MODIFIERS) do
                if NPC.HasModifier(target, mod) then return true end
            end
            return NPC.HasState(target, MS.MODIFIER_STATE_CHANNELED)
        end)(),
        ...
        setup_killable    = setup_killable,
        q_kill_floor      = q_kill_floor,
        atk_range_with_e  = atk_range + take_aim_range_bonus(),
        atk_range         = atk_range,
        ...
        -- v6.15.42: per-hero offensive-kite preference. User feedback:
        -- 'For phantom assassin approach we should use pike first and
        -- grenade as fall back; grenade is sending characters on random
        -- positions.' Grenade's radial knockback from cast point is
        -- direction-deterministic at the macro scale but visually feels
        -- random when target is near cast center. Pike's push is along a
        -- predictable Sniper→target line. For specific enemies (blink-
        -- melee carries that pop in close), pike is the better tool.
        kite_prefers_pike = (function()
            local name = NPC.GetUnitName and NPC.GetUnitName(target) or ""
            return OFFENSIVE_KITE_HERO_OVERRIDES[name] == "pike"
        end)(),
        ...
        in_teamfight = in_teamfight_v,
        -- v6.15.72: target state projected forward 2s (matches rc_2s autos
        -- window). Used by q_chain_step_cond to verify "Q1+autos closes the
        -- kill" against PROJECTED eff_hp (current eff_hp + regen+heals over
        -- 2s) rather than dispatch-time eff_hp — prevents over-fire on
        -- targets that heal during the autos window. Also exposes range
        -- prediction (will target leave atk/q/r range during the window?).
        -- v6.15.76: now also consumed by snipe_e_r / snipe_q_r / snipe_standard
        -- commit_preds for range-aware autos contribution + R-impact eff_hp.
        proj_state_2s = project_target_state(target, 2.0),
        -- v6.15.76 (user directive: "apply target-state extrapolation model
        -- to combos and decision"): live R cast point (Scepter-aware).
        -- Without Scepter: 2.0s. With Scepter: 0.5s (per LIQUIPEDIA_REF.md).
        -- Used as projection horizon for proj_state_r_impact below.
        r_cast_s = r_cast_point(),
        ...
        -- v6.15.79 (QA fix from v6.15.76): autos-window misalignment fix.
        -- v6.15.76 used proj_state_2s.in_atk_range for "will target be in
        -- RC during the autos window" — but autos actually fire DURING
        -- [r_cast_s, r_cast_s + 2s] (Sniper is frozen during R cast).
        -- For non-Scepter (r_cast_s = 2.0s), the 2s projection window
        -- missed the actual autos window by 2s. proj_state_post_r projects
        -- to r_cast_s + 2s (end of autos window) so the in_atk_range check
        -- predicts target's position when autos START (~r_cast_s) through
        -- when they END. Combo commit_preds now read this instead.
        proj_state_post_r = project_target_state(target, r_cast_point() + 2.0),
        ...
        self_ms_slow_pct = (function()
            local _, pct = self_take_aim_state()
            return pct
        end)(),
        -- v6.15.52 (G4): elapsed seconds since first enemy entered scan
        -- radius. 0 means no fight active (no enemies in range). Used by
        -- combo commit_preds to refuse R on a full-HP target during the
        -- first ~2s of an engagement — initiation belongs to the team,
        -- Sniper's R is the cleanup seal.
        fight_age_s = (state.fight_start_t or 0) > 0
            and (now() - state.fight_start_t) or 0,
        ...
        -- v6.15.53 (G8 — enemy active heal): target has Satanic / Bloodthorn
        -- /etc. lifesteal-burst active. During R's 2s cast point, target
        -- heals ~600 HP from a Satanic Unholy Rage (~175%% lifesteal, 3 autos).
        -- eff_hp_magical gets bumped 500 HP to reflect the survival margin.
        -- v6.15.198 (audit C3): reads the same cached HasModifier values
        -- the eff_hp_magical bump above uses — the inline closure here
        -- was duplicating those 2 engine calls per ctx build.
        target_active_heal = (target_has_satanic_active and 500)
                              or (target_has_bloodthorn_active and 300)
                              or false,
        ...
        -- v6.15.91: also expose r_physical (instant-attack instance) so the
        -- combo_dmg_breakdown log can show magical-vs-physical split.
        r_dmg_at_d        = r_dmg_at_d,
        r_physical_at_d   = r_physical,
        rc_2s             = rc_2s,
        ...
        -- v6.10 G4: cluster detection for Scepter AoE R.
        cluster_count     = (function()
            local n, _ = cluster_around_target(target, me)
            return n
        end)(),
        cluster_worth_aoe = (function()
            local _, worth = cluster_around_target(target, me)
            return worth and NPC.HasScepter(me)
        end)(),
...
-- dispatch-time ctx so cond_fn at fire time can still access q_kill_floor,
-- eff_hp, setup_killable, etc. At fire time we override `target_pos` and
-- `q_charges` with live values (cheap re-reads) while keeping the rest
-- as dispatch-time snapshots. Aborts if target died/became invalid.
...
local function pending_steps_tick()
    ...
    for i = 1, #state.pending_steps do
        ...
        elseif not self_alive_ok() then
            ...
            -- v6.15.72 (QA-fix-2): refresh proj_state_2s at fire time so
            -- the heal_delta projection window is "next 2s from NOW", not
            -- "next 2s from dispatch." Without this, q2 firing at +1.5s
            -- post-dispatch uses a heal-delta computed from dispatch time,
            -- which over-credits healing already realized in live.eff_hp
            -- (~50% over-estimate at q2 fire, ~100% at q3 fire). Cheap:
            -- ~6 API calls reusing already-loaded target handle.
            if p.target and Entity.IsEntity(p.target) then
                live.proj_state_2s = project_target_state(p.target, 2.0)
            end
            ...
            if p.cond_fn and not p.cond_fn(live) then
                ...
                if p.kind == "ut" then
                    ...
                    -- v6.15.96: parity with fire_steps line ~3032 for R cast
                ...
                -- Mirror of the fire_steps update at ~line 3117. Skips item
...
local function r_abort_tick()
    ...
    if not Ability.IsInAbilityPhase(r) then
        -- v6.15.218: the v6.15.217 per-tick R re-issue was REMOVED. It
        -- re-issued R queue=false every tick while R was not yet in phase,
        -- which cancelled the dr combo's D mid-cast-point. R reliability is
        -- restored instead by the original deferred dispatch (delay_s = 0.2
        -- + issue_cast_target's execute_fast=true) -- see changelog v6.15.218.
        -- v6.15.141: FAST R-cancel detection. If R was dispatched but has
        -- still not entered its cast phase R_PHASE_START_DEADLINE seconds
        -- later (and was never seen in phase), the engine cancelled it on
        -- order receipt — a native/user MOVE or ATTACK replaced it before
        -- the cast point even started. Without this the brain assumes R is
        -- mid-cast and stays throttle-locked the full 2.5s before retrying
        -- (user: "R outside AA range took a while to fire even off cooldown").
        -- Clear the R-in-flight markers and downgrade the dispatch throttle
        -- from the 2.5s R window to the 0.4s SEQ window so starter_tick /
        -- teamfight_tick re-appraise and retry R within ~1s. R was cancelled,
        -- not cast, so it is NOT on cooldown — the retry can fire it.
        if state.last_r_dispatch_t > 0 and not state.r_phase_seen
           and (now() - state.last_r_dispatch_t)
               > state.R_PHASE_START_DEADLINE then
            local tgt   = state.last_r_target
            local cname = state.last_r_combo_name
            local delta = now() - state.last_r_dispatch_t
            tlog(1, "r_cast_never_started", {
                target = uname(tgt),
                combo  = cname or "?",
                age_ms = string.format("%.0f", delta * 1000),
                retry  = state.r_retry_count,
            })
            -- v6.15.226: R was cancelled in its pre-phase by the native
            -- order flood (execute_fast only beats SAME-tick natives; the
            -- OnPrepareUnitOrders veto never sees native subsystem orders).
            -- Re-dispatch R, bounded by R_RETRY_MAX, rather than only
            -- clearing the markers and waiting ~1s for a re-appraisal that
            -- usually misses the window. Bounded, NOT the per-tick re-issue
            -- v6.15.217 used (that cancelled D); gated on R provably never
            -- entering its phase. r is the function-local from line ~3303.
            local can_retry = tgt and Entity.IsEntity(tgt)
                              and Target.IsAlive(tgt)
                              and state.r_retry_count < state.R_RETRY_MAX
            if can_retry
               and issue_cast_target("retry_" .. (cname or "r"), r, tgt) then
                state.r_retry_count          = state.r_retry_count + 1
                state.last_r_dispatch_t      = now()
                state.r_cast_protect_until_t = now() + r_cast_point() + 0.4
                state.r_phase_seen           = false
                -- Re-anchor the combo's deferred Q/E by the same slip so
                -- they still fire after the retried R completes; an
                -- un-shifted Shrapnel landing mid-cast would cancel R.
                for i = 1, #state.pending_steps do
                    local p = state.pending_steps[i]
                    if p.combo_name == cname then
                        p.fire_at = p.fire_at + delta
                    end
                end
                tlog(1, "r_cast_retry", {
                    target  = uname(tgt),
                    combo   = cname or "?",
                    attempt = state.r_retry_count,
                })
                return
            end
            -- Budget spent / target gone: clear and let starter_tick or
            -- teamfight_tick re-appraise (the pre-v6.15.226 behaviour).
    ...
    -- v6.15.141: R is confirmed in its cast phase — record it so the
    -- fast-cancel detector above does not misfire (a real cast that simply
    -- took >R_PHASE_START_DEADLINE to start would otherwise look cancelled).
    -- v6.15.226: this R-chain reached its cast phase; reset the retry budget.
    ...
    for i = 1, #steps do
        ...
        elseif step.cond and not step.cond(ctx) then
            ...
            if step.kind == "ut" then
                ...
                -- v6.15.223: a step may opt out via no_fast. heavy_starter
                -- issues E then R same-tick; execute_fast on R jumped it
                -- ahead of the E issued just before and inverted the tap to
                -- R-then-E. Its R step sets no_fast so E locks its cast first.
                ok = safe_issue {
                    hero = HERO_KEY, layer = "agg", intent = intent,
                    order_type = UO.DOTA_UNIT_ORDER_CAST_TARGET,
                    unit = state.self_npc, ability = ability(step.ability),
                    target = arg, queue = use_queue,
                    execute_fast = (step.ability == A.R) and not step.no_fast,
                }
            ...
            -- are read by auto_grenade_tick (line ~7600) to gate "don't re-fire
            ...
            -- update in pending_steps_tick (~line 2883) for deferred steps.
            if ok then
...
-- v6.14.1 C1: assign to the forward-declared local (line 53) instead of
-- creating a new shadowing local. `local function` would otherwise create a
-- fresh local that doesn't propagate up to build_layer1_ctx's closure.
cluster_around_target = function(target, me)
    if not target or not me then return 0, false end
    local list = Entity.GetHeroesInRadius(target, 400, Enum.TeamType.TEAM_FRIEND)
    -- TEAM_FRIEND from target's POV = target's own team = our enemies.
    if not list then return 0, false end
    -- v6.14.1 low: exclude target itself. GetHeroesInRadius from target's POV
    -- includes target in their own friend list — count would have been
    -- target + nearby. Off-by-one vs comment ("cluster of 2+ enemies").
    local target_idx = Entity.GetIndex(target)
    local count = 1                        -- target is always 1 of the cluster
    local bkb_count = NPC.HasState(target, MS.MODIFIER_STATE_MAGIC_IMMUNE) and 1 or 0
    for i = 1, #list do
        local h = list[i]
        if h and Entity.GetIndex(h) ~= target_idx
           and Target.IsAlive(h) and Target.NotClone(h) then
            count = count + 1
            if NPC.HasState(h, MS.MODIFIER_STATE_MAGIC_IMMUNE) then
                bkb_count = bkb_count + 1
            end
        end
    end
    -- "Worth Scepter AoE" = cluster of 2+, no more than 1 BKB'd.
    return count, (count >= 2 and bkb_count <= 1)
end

-- v6.15.42/.43: OFFENSIVE_KITE_HERO_OVERRIDES is declared above
-- live_q_kill_floor / build_layer1_ctx so it's in lexical
-- scope when build_layer1_ctx reads it via c.kite_prefers_pike. Was
-- mistakenly redeclared here in v6.15.42 — removed in v6.15.43.
...
-- v6.15.128: optimal Concussive Grenade (D) placement for the dr combo. D
-- knocks units in its blast AWAY from the grenade's centre — so to PEEL an
-- attacker off Sniper, D must land on the Sniper→attacker line, offset inward
-- from the attacker so the attacker sits near the far edge of the blast and
-- is shoved straight outward (away from Sniper). Casting D AT the attacker
-- (centred) gives a poor/undefined knockback direction. The grenade is placed
-- D_PEEL_OFFSET (~300u) inward from the attacker — inside D's 375u effect
-- radius. A melee attacker (close) → g clamps to 0 → D lands on Sniper's own
-- position; a ranged attacker (far) → g grows toward D's cast range. Clamped
-- to D's cast range. Returns target_pos as a safe fallback.
state.optimal_d_pos = function(c)
    local me = state.self_npc
    local mp = (me and Entity.IsEntity(me)) and Entity.GetAbsOrigin(me) or nil
    ...
    if g < 0 then g = 0 elseif g > cast_d then g = cast_d end
...
state.heavy_starter_tick = function(force)
    ...
    -- v6.15.120 (user empirical, demo of v6.15.119): E and R fire SAME-TICK
    -- as two fresh queue=false dispatches, E first. v6.15.119 deferred R by
    -- delay_s=0.05, but pending_steps_tick is tick-quantized (~15Hz) so R
    -- actually landed ~0.067s behind E (log: E ExecuteOrder t=48.300, R
    -- t=48.367) — over the user's "≤0.05s" ceiling — and R was cancelled
    -- (cast_verify fired=n). Same-tick is the only way to guarantee ≤0.05s.
    -- E is an instant cast (0 cast point) so it resolves immediately; R,
    -- issued right after with no_queue=true, then casts before the native
    -- positioning subsystem can react to Take Aim and emit the R-cancelling
    -- MOVE. NOT a queue chain (R queued behind E) — that makes the 2nd step
    -- unreliable (v6.15.103: E-first queue chain → R 0/6 fired).
    -- v6.15.135: E is cond-gated on ready_e. When E is on cooldown the step
    -- skips and R fires alone (still queue=false via no_queue) — the tap
    -- always delivers R. When E IS ready it fires first, same-tick, exactly
    -- as before.
    local steps = {
        { ability = A.E, kind = "nt", short = "e",
          cond = function(c) return c.ready_e end },
        { ability = A.R, kind = "ut", short = "r", no_queue = true,
          -- v6.15.223: NOT execute_fast. R is issued same-tick right after
          -- E; without this, execute_fast (v6.15.220) jumps R ahead of E
          -- and the tap fires R-then-E. E (instant) must lock its cast first.
          no_fast = true,
...
state.tf_team_focus = function(enemy_list)
    -- v6.15.195 (audit A2): throttled instrumentation. The bridge open
...
state.teamfight_tick = function(force)
    ...
    --     api/npc.md:442), already used at lines 1225, 1590 to drive
    ...
    if not steps then
        ...
        -- (line 4662) so the diagnostic does not lie about whether R would
...
local function on_enemy_channel_start(caster, mod_name, target_self)
    ...
    -- never fire on the L1.5 target and (b) layer1_tick can dispatch a
    -- competing R the next tick because last_layer1_t isn't set.
    -- v6.14.1 H4: use a SEPARATE state.last_layer15_t throttle so L1.5 fires
    -- don't block user's combo-key dispatch for 2.5s afterward.
       ...
       and dist_to(caster) <= state.grenade_cast_range() then
        if not NPC.HasState(caster, MS.MODIFIER_STATE_MAGIC_IMMUNE) then
            ...
            if grenade_fn then
                ...
                local pos = Entity.GetAbsOrigin(caster)
...
local function mark_layer2_fired() state.last_save_t = now() end
...
local function record_save(intent, item_name, threat_mod)
...
state.pike_self_reposition = function(intent, threat_caster, it)
    ...
    if not me then return false end
    local me_pos = Entity.GetAbsOrigin(me)
    local cp     = Entity.GetAbsOrigin(threat_caster)
    if not me_pos or not cp then return false end
    local away = me_pos - cp
    if away:Length2DSqr() < 1 then return false end  -- caster on top of Sniper
    away = away:Normalized()
    local away_pt = Vector(me_pos.x + away.x * 400, me_pos.y + away.y * 400, me_pos.z)
...
local SAVE_FIRE = {
    ...
    item_hurricane_pike = {
        ...
        fire  = function(intent, threat_caster, threat_mod)
            ...
            -- same two-phase mechanism drains use. Raw self-cast only when
            -- the caster is unknown (no away-direction to compute).
            if threat_caster and Entity.IsEntity(threat_caster)
               and Target.IsAlive(threat_caster) then
                return state.pike_self_reposition(intent, threat_caster, it)
            end
            local ok = issue_item_self(intent, "def", it)
            if ok then record_displacement(0, now() + 0.5) end  -- self airborne
            return ok
        end,
    },
    ...
    grenade_at_caster   = {
        ...
        fire  = function(intent, threat_caster)
            ...
            if not me then return false end
            local me_pos     = Entity.GetAbsOrigin(me)
            local caster_pos = Entity.GetAbsOrigin(threat_caster)
            ...
            -- v6.15.214: cast the grenade ON the enemy (its predicted
            -- position), not at the Sniper-caster midpoint. The 0.4s stun +
            -- knockback BREAKS the enemy (interrupts Bara charge, Shaman
            -- channel cast) and makes the kill easier -- hitting the enemy is
            -- the priority. A moving caster is still LED: cast at predict_pos
            -- over lead_s (turn time + cast point + projectile travel). The
            -- cast point is clamped to grenade cast range so the order stays
            -- valid; a stationary caster predicts to its own position.
            ...
            local cx, cy = caster_pred.x, caster_pred.y
...
local SNIPER_SAVE_OVERRIDES = {
    ...
    -- PA Phantom Strike (reactive — blink completed). Pike pushes PA
    -- 425u outward; Glimmer breaks target-lock.
    ...
    for _, save_name in ipairs(order) do
        ...
        elseif not save_is_ready(save_name) then
            ...
            if penalty < -20 then
                ...
                if ok then
                    record_save(intent, fire_entry.short, threat_mod)
...
local function save_channel_on_self(caster, threat_mod)
    ...
    local pos = Entity.GetAbsOrigin(caster)
    if issue_cast_position("channel_on_me_d", ability(A.D), pos) then
        record_save("channel_on_me_source", "grenade_at_caster")
    end
end
...
local function pre_face_tick()
    ...
    local angle = math.abs(NPC.FindRotationAngle(me, best_pos))
...
local function brain_native_diagnostic_tick()
    ...
    if t_now - state.native_window_start >= 5.0 then
        ...
        -- the user can fix it. See HERO_PROMPT.md #67 (Option A setup).
...
-- abort (line ~2820 cast_abort_tick / cast_verify path).
...
local function auto_grenade_tick()
    ...
    -- Find nearest valid enemy hero in trigger radius
    local me_pos       = Entity.GetAbsOrigin(me)
    ...
    for i = 1, #heroes do
        ...
        if h ~= me and Entity.IsAlive(h) and not Entity.IsSameTeam(me, h) then
            ...
            if ch then
                ...
                if an and an ~= "" then
                    local d = me_pos:Distance2D(Entity.GetAbsOrigin(h))
                    if d < 1300 then
                        tlog(1, "channel_live", {
                            caster    = uname(h),
                            ability   = an,
...
function callbacks.OnUpdateEx()
    ...
    -- the heartbeat block emits a config_warn — see line ~5800 region.
...
function callbacks.OnModifierCreate(npc, modifier)
    ...
    -- at line 8674). Modifier.GetName(nil) is undefined; engine has been
...
function callbacks.OnLinearProjectileCreate(data)
    ...
    local me_pos = Entity.GetAbsOrigin(state.self_npc)
    local origin = data.origin or Entity.GetAbsOrigin(src)
    local velocity = data.velocity
    if not velocity or not origin then return end
...
refresh_status_panel = function()
    ...
    if m.debug_panel and m.debug_panel:Get() then
        ...
        local tp = t and Entity.GetAbsOrigin(t)
        if t and tp then
