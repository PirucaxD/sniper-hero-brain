...
state.R_PHASE_START_DEADLINE = 1.5 -- v6.15.229: per-frame R re-issue give-up cap. r_abort_tick re-issues the R cast order every tick while R has not entered its cast phase, until it locks or this many seconds elapse, then gives R up as cancelled. Retuned from v6.15.228's 0.35: a coupled-config test showed R locks about 1s in, so 0.35 expired before the lock and mislogged r_cast_never_started 7-of-7 while R actually fired. The re-issue stops the instant R locks; this is only the hopeless-case give-up.
