...
state.R_PHASE_START_DEADLINE = 0.35 -- v6.15.228: per-frame R re-issue window. r_abort_tick re-issues the R cast order every tick while R has not entered its cast phase; if R still has not by this many seconds after first dispatch it is given up as cancelled. Tight: the native script locks R in under 0.2s of per-frame re-issue. (Was 0.6 for the v6.15.226 bounded retry; v6.15.217 had 1.0.)
state.r_reissue_count        = 0   -- v6.15.228: per-frame R re-issues on the current attempt; for the r_cast_locked / r_cast_never_started log lines, reset on lock or give-up.
...
local function r_abort_tick()
    ...
    if not Ability.IsInAbilityPhase(r) then
        -- v6.15.228: per-frame R re-issue, modeled on the native Sniper
        -- script. A native-only log showed the native re-fires its R cast
        -- order every ~33ms until R locks (bursts of up to 6). While R has
        -- been dispatched but has not entered its cast phase, re-issue the
        -- cast order every tick via a bare Ability.CastTarget (execute_fast,
        -- no safe_issue so no per-tick cast_verify spam) until R locks or
        -- R_PHASE_START_DEADLINE elapses. Safe where v6.15.217's per-tick
        -- re-issue was not: that ran while D was still in its cast point (R
        -- was un-deferred then); now the dr R is deferred 0.2s (v6.15.218),
        -- D's 0.1s cast point finishes before R's first dispatch, and this
        -- branch only runs after that, so a re-issue cannot touch a done D.
        -- Q/E are still future entries in pending_steps. On lock the slip is
        -- re-anchored (see the r_phase_seen transition below). Supersedes
        -- the v6.15.226 bounded-2 retry.
        if state.last_r_dispatch_t > 0 and not state.r_phase_seen then
            ...
            if age < state.R_PHASE_START_DEADLINE
               and tgt and Entity.IsEntity(tgt) and Target.IsAlive(tgt) then
                if Ability.CastTarget then
                    Ability.CastTarget(r, tgt, false, false, true)
                end
                state.r_reissue_count = (state.r_reissue_count or 0) + 1
                return
            end
            -- Window elapsed (or target gone): R never entered its phase.
