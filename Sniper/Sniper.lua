...
local WORTHY_CHANNEL_ABILITIES = TD.WORTHY_CHANNEL_ABILITIES
...
local function target_in_channel(t)
    ...
    -- v6.15.276 (Option A): the v6.15.199 generic GetChannellingAbility
    -- catch awarded the +200 channel-interrupt bonus to ANY channel,
    -- including low-value player-releasable ones (KOTL Illuminate releases
    -- on command and finishes before R's ~2s cast lands -- R wasted). Now
    -- the generic path only scores the bonus when the channelling ability
    -- is in the curated WORTHY_CHANNEL_ABILITIES allowlist. An uncatalogued
    -- channel returns nil (no interrupt bonus); a killable channeler still
    -- draws R via the kill bonus. Returns the ability name (not a real
    -- modifier name) -- downstream only special-cases "modifier_teleporting"
    -- for the +50, so the bare +200 path is unaffected.
    if NPC.GetChannellingAbility then
        local cab = NPC.GetChannellingAbility(t)
        if cab then
            local cn = Ability.GetName(cab)
            if cn and WORTHY_CHANNEL_ABILITIES[cn] then
                return cn
            end
            return nil
        end
    end
    -- Legacy fallback (GetChannellingAbility unavailable): match the curated
    -- modifier catalog directly on the unit. The MODIFIER_STATE_CHANNELED
    -- state check (v6.15.199) is dropped as a +200 source -- it was the same
    -- generic over-catch as the live_channel path.
...
local function ScoreUltTarget(target)
    ...
    -- v6.15.277: Oracle False Promise and Dazzle Shallow Grave defeat R
    -- commits during their windows. Shallow Grave hard-floors the
    -- target's HP at 1 for 4-5.5s (undispellable, no Sniper-kit way
    -- through); False Promise delays/negates damage and doubles healing
    -- for 7-10s (undispellable, Oracle's heal often saves). Both make R
    -- wasteful for "kill now" -- the cooldown is gone for a kill that
    -- does not materialize. -200 (Option 2) is harsher than Aegis (-75)
    -- because Aegis lets R kill the target temporarily (then respawn);
    -- these two prevent the kill outright. Force key still bypasses
    -- score, so the user can force R if they really want. Modifier names
    -- verified pending live log observation (modseen catalog convention).
    if NPC.HasModifier(target, "modifier_dazzle_shallow_grave") then
        score = score - 200
    end
    if NPC.HasModifier(target, "modifier_oracle_false_promise") then
        score = score - 200
    end
...
local function recompute_candidates()
    ...
    if scored[1] then
        ...
        -- v6.15.277: mirror Shallow Grave / False Promise penalties.
        if NPC.HasModifier(t, "modifier_dazzle_shallow_grave") then
            parts[#parts + 1] = "shallow_grave-200"
        end
        if NPC.HasModifier(t, "modifier_oracle_false_promise") then
            parts[#parts + 1] = "false_promise-200"
        end
...
state.starter_tick = function(force)
    ...
    -- v6.15.275: removed the v6.15.251 per-target save-cooldown gate
    -- (STARTER_SAVE_SUPPRESS_S, 100ms). The gate suppressed offensive
    -- starter dispatch for 100ms after any defensive save on the same
    -- target. Its stated purpose (prevent D+R double-fire) was actually
    -- handled elsewhere by the save chain first-success-wins logic
    -- (one defensive save per threat). The gate was blocking the
    -- legitimate D+R+Q+E offensive combo when combo_key was held on a
    -- committed attacker immediately after the defensive pike or
    -- grenade fired, exactly when the user wants R to commit.
    -- STARTER_SAVE_SUPPRESS_S plus state.last_save_t / last_save_target
    -- stamps left in place (cheap, no consumers post-removal; separate
...
local function on_enemy_channel_start(caster, mod_name, target_self)
    ...
    -- v6.15.262: track whether R fires so we can skip the grenade interrupt
    -- below. User report on v6.15.261 demo (CM Freezing Field): both R and
    -- grenade-at-caster fired on the same channel. Either save alone
    -- interrupts the channel; firing both wastes mana + cooldown + can
    -- cause the cast-order overlap the user observed. R is preferred when
    -- it commits (kills target = interrupts channel cleanly); grenade
    -- is the fallback when R doesn't commit (target above kill threshold
    -- OR R on cooldown OR target magic-immune).
    local r_committed = false
    ...
    -- v6.15.262: skip when R was just committed -- R will kill target and
    -- interrupt the channel; grenade would be redundant.
    if r_committed then
        tlog(3, "channel_grenade_skip_r_committed", { caster = uname(caster) })
        return
    end
...
local SAVE_FIRE = {
    ...
    grenade_at_caster   = {
        ...
        fire  = function(intent, threat_caster, threat_mod)
            ...
            -- v6.15.255: gate scoped to channel_on_self threats only. For
            -- close_gap blink/charge/melee (PA / Bara / Tusk / Marci /
            -- Primal Beast) Sniper is NOT stunned mid-turn -- the engine
            -- completes rotation and the cast lands. The v6.15.254 log
            -- showed 4/5 PA blink events tripping the gate (angle 141 to
            -- 170 > 120) -- chain fell through to grenade_self with
            -- pick_escape_dir bias, producing "seems random" placements.
            -- User: "if D is cast on the right position sniper will turn
            -- himself alone without intervention". Per-category gate
            -- preserves the Pudge / Bane / Shaman protection without
            -- punishing non-stun threats. nil threat_mod (panic mode,
            -- channel_break callsite) drops the gate -- trust the engine.
            local angle_to_cast = math.deg(math.abs(NPC.FindRotationAngle(me, cast_point)))
            local category = (threat_mod and TD.CategoryOf(threat_mod)) or "-"
            local gate_active = category == "channel_on_self"
            tlog(3, "grenade_at_caster_cast_plan", {
                ...
                category = category,
                gate_active = gate_active and "y" or "n",
            })
            if gate_active and angle_to_cast > MAX_TURN_FOR_GRENADE_AT_CASTER then
...
local SNIPER_SAVE_OVERRIDES = {
    ...
    -- v6.15.256: Underlord Pit of Malice. 400u-radius snare pit, 1.5-1.8s
    -- root per tick, re-snares every 3.6s for 12s. Same trap-escape posture
    -- as Disruptor Kinetic Field: only knockback (grenade self/at-caster,
    -- pike, force) moves Sniper out reliably. The pit's persistent ticks
    -- mean a single successful escape covers the rest of the duration as
    -- long as Sniper stays out -- chain doesn't need to re-fire per snare.
...
state.ANIM_SAVE_OVERRIDES = {
    ...
    abyssal_underlord_pit_of_malice   = SNIPER_SAVE_OVERRIDES.modifier_abyssal_underlord_pit_of_malice_ensare,
}
...
-- v6.15.259: CATEGORY_CHAINS extracted to lib/threat_data.lua.
-- v6.15.260: lib chains are now hero-agnostic (items only); Sniper inserts
-- its Concussive Grenade saves via SNIPER_CATEGORY_PATCHES below using
-- {prepend, insert_after, append} semantics. Patches are applied once at
-- module load to produce the effective CATEGORY_CHAINS table, which the
-- resolver uses in place of the raw lib table. This keeps the lib reusable
-- by other hero brains (Underlord, etc.) while preserving the previous
-- Sniper behaviour.
--
-- Patch semantics per entry:
--   prepend      = {save1, save2, ...}  -- inserted at the start
--   insert_after = "<save_name>"        -- inserted right after this lib entry
--   insert       = {save1, ...}         -- the saves to insert at insert_after
--   append       = {save1, save2, ...}  -- inserted at the end
local SNIPER_CATEGORY_PATCHES = {
    -- Close-gap (PA, Bara, Tusk, Marci, Primal Beast Onslaught etc.).
    -- Grenade-at-caster's stun + knock interrupts the gap-close and pushes
    -- the attacker away. Inserted right after Pike so Pike-on-enemy fires
    -- first when in range (radial 425u push moves the charger away); if
    -- Pike is unavailable, grenade fires before falling to Force/BKB.
    -- Matches the v6.15.259 close_gap order exactly.
    close_gap = {
        insert_after = "item_hurricane_pike",
        insert       = { "grenade_at_caster" },
    },
    -- Channel-on-self (Pudge Dismember, Bane Grip, Shaman Shackles, Lich
    -- Sinister Gaze, PB Pulverize, Grimstroke Soul Chain, Oracle Fortunes
    -- End channel). Grenade-at-caster's 0.4s stun + push breaks the tether
    -- fastest -- goes FIRST (winning the 0.5s race against Pudge's 0.3s
    -- stun, which the v6.15.46 incident validated). grenade-self is the
    -- panic fallback when grenade-at-caster cannot reach.
    channel_on_self = {
        prepend = { "grenade_at_caster" },
        append  = { "grenade_self" },
    },
    -- Line projectiles. grenade_self pushes Sniper perpendicular off the
    -- line; goes right after Pike so Pike fires when in range and grenade
    -- handles the line dodge otherwise.
    line_projectile = {
        insert_after = "item_hurricane_pike",
        insert       = { "grenade_self" },
    },
    -- Traps (Kinetic Field, Pit of Malice). Forced movement blocked --
    -- knockback only escapes. Sniper's grenade pushes him out reliably;
    -- goes first so the grenade fires before lib's blink fallbacks
    -- (Sniper rarely owns blink). Matches the v6.15.259 trap chain
    -- (which had only the two grenades).
    trap = {
        prepend = { "grenade_self", "grenade_at_caster" },
    },
    -- Drain channels (Pugna Life Drain, Lion Mana Drain). grenade_at_caster
    -- 0.4s stun interrupts the channel; goes right after Pike (the canonical
    -- tether-breaker). grenade-self is the displacement fallback.
    drain = {
        insert_after = "item_hurricane_pike",
        insert       = { "grenade_at_caster" },
        append       = { "grenade_self" },
...
local function _apply_category_patch(base, patch)
    ...
    for i = 1, #base do
        out[#out + 1] = base[i]
        if patch.insert_after and patch.insert
           and base[i] == patch.insert_after
        then
...
do
    for cat, lib_chain in pairs(TD.CATEGORY_CHAINS or {}) do
        CATEGORY_CHAINS[cat] = _apply_category_patch(
            lib_chain, SNIPER_CATEGORY_PATCHES[cat])
    end
end

-- v6.15.261: per-modifier patches into lib.RECOMMENDED_SAVES. Same merge
-- semantics as SNIPER_CATEGORY_PATCHES (prepend / insert_after+insert /
-- append). Used to inject Sniper's grenade saves into the lib's now-
-- items-only RECOMMENDED_SAVES entries. Effective chains match v6.15.260
-- for all entries where Sniper had grenade_self / grenade_at_caster
-- inline.
--
-- Modifiers ALREADY covered by SNIPER_SAVE_OVERRIDES (above) bypass this
-- layer -- their override wins authoritative in resolve_save_order. The
-- patches below cover the un-overridden modifiers where the lib's chain
-- is the actual dispatch chain.
local SNIPER_THREAT_PATCHES = {
    -- Channels -- grenade-self joins as panic fallback.
    modifier_pudge_dismember_pull = {
        prepend = { "grenade_self" },
    },
    -- Delayed AoEs / line projectiles -- grenade-self pushes Sniper off.
    modifier_kez_grappling_claw_slow = {
        insert_after = "item_force_staff",
        insert       = { "grenade_self" },
    },
    ...
    -- Drain channels -- grenade-self displaces.
...
do
    -- Pre-compute every (lib chain x patch) merge at load. The set of
    -- modifiers is the union of TD.RECOMMENDED_SAVES keys (lib has a
    -- chain) and SNIPER_THREAT_PATCHES keys (Sniper has a patch). When
    -- only one side has the entry, the other side contributes an empty
    -- chain / no patch and the merge yields the present side verbatim.
    local seen = {}
    for mod, _ in pairs(TD.RECOMMENDED_SAVES or {}) do seen[mod] = true end
    for mod, _ in pairs(SNIPER_THREAT_PATCHES)       do seen[mod] = true end
    for mod, _ in pairs(seen) do
        local base  = (TD.RECOMMENDED_SAVES and TD.RECOMMENDED_SAVES[mod]) or {}
        local patch = SNIPER_THREAT_PATCHES[mod]
        PATCHED_RECOMMENDED_SAVES[mod] = _apply_category_patch(base, patch)
    end
end
...
local function resolve_save_order(threat_mod, category_hint, ability)
    ...
    if threat_mod then
        ...
        -- v6.15.261: PATCHED_RECOMMENDED_SAVES is the lib's RECOMMENDED_SAVES
        -- merged with SNIPER_THREAT_PATCHES at module load. Same effective
        -- chain as v6.15.260's TD.RecommendedSaves call, with hero-specific
        -- grenade saves re-injected. For modifiers in lib but without a
        -- Sniper patch, the entry equals the lib chain verbatim.
        local td = PATCHED_RECOMMENDED_SAVES[threat_mod]
        if td and #td > 0 then return td, false end
...
local function register_anim_maps()
    ...
    -- Aphotic Shield cast on ally explodes for AoE damage; anim catches
    -- the cast, Sniper repositions if Abaddon's ally is near.
    ...
    -- Sticky Snare is POINT VECTOR_TARGETING CHANNELLED -- Brood places a
    -- web-snare that roots Sniper if they walk through. anim catches the
    -- cast.
    ...
    -- Double Edge is UNIT_TARGET AOE instant burst; no Sniper modifier.
...
-- v6.15.274: hero-gate the entire callbacks table after the lib .Wire calls
-- have built the chain. UCZone loads every script in scripts/ for every
-- match regardless of which hero the player picks (no framework auto-gate).
-- Without this, Sniper attached to a Drow match and fired pike twice on PA
-- (debug.log lines 311-323 and 886-945). Outermost wrap covers Sniper's 6
-- callbacks plus 5 lib-chained handlers (Order, Damage, Anim) and any future
-- .Wire additions. Per-frame dedup in lib handlers keeps multi-hero installs
-- cheap: only the active hero chain fires lib work each frame; gated chains
-- are no-ops.
