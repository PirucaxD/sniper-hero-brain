...
--
-- Algorithm: try 5 angles off the straight-away vector (0, -35, 35, -65,
-- 65 degrees), keep the candidate with the lowest danger_at_pos that
-- still passes SafePushDestination for the given threat hint. Ties go to
-- 0deg (the baseline) via strict less-than, so a marginal angle does not
-- win over the straight-away unless meaningfully safer. Returns
-- (escape_dir, landing) where escape_dir is the unit vector FROM Sniper
-- TO the chosen landing; (nil, nil) if every candidate failed terrain or
-- the threat-distance check.
