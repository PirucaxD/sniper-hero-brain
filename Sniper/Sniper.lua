        ...
        if state.last_r_dispatch_t > 0 and not state.r_phase_seen
           and (now() - state.last_r_dispatch_t)
            ...
            tlog(1, "r_cast_never_started", {
                ...
                age_ms = string.format("%.0f",
                    (now() - state.last_r_dispatch_t) * 1000),
            })
    ...
    for i = 1, #steps do
        ...
        elseif step.cond and not step.cond(ctx) then
            ...
            if step.kind == "ut" then
                ...
                -- and `tf_r` single-step R finishers, and heavy_starter's R)
                -- gets execute_fast, matching the deferred path's
                -- issue_cast_target. execute_fast bypasses internal safety
                -- delays so R's order is processed ahead of the native
                -- subsystem orders queued the same tick (v6.15.95 rationale).
                -- Before this, only the deferred dr-combo R was protected;
                -- the inline `r` kill finisher and `tf_r` raced the flood
                -- unaided (v6.15.219 log: starter_r_r 1/2). Scoped to A.R so
                -- non-critical ut casts are not accelerated.
                ok = safe_issue {
                    hero = HERO_KEY, layer = "agg", intent = intent,
                    order_type = UO.DOTA_UNIT_ORDER_CAST_TARGET,
...
state.predict_pos = function(target, lead_s)
    ...
    return Geom.lead_target_pos(target, state.self_npc, lead_s) or tpos
end
        ...
        if not state.last_queue_snapshot[key] then
            ...
            if entry.abilityIndex and entry.abilityIndex > 0 and Entity.Get then
                ...
                if ab and Ability and Ability.GetName then
                    ability_name = Ability.GetName(ab) or "?"
                end
            end
            local target_name = "-"
