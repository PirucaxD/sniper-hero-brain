...
local function grenade_self_cast_point(threat_caster_hint)
    ...
    -- v6.15.244 (clue C4 finalisation): danger-aware landing pick. Try 5
    -- angles off straight-away and use the lowest-danger candidate that
    -- still passes SafePushDestination. 0deg (the historical behaviour)
    -- ties go to the baseline via strict less-than. If every candidate
    -- fails SafePushDestination (cliff / pinned against terrain / closer
    -- to threat in every direction), the helper returns nil and the chain
    -- falls through to the next save.
    local escape_dir, landing = state.pick_escape_dir(me_pos, toward_threat, push_distance, threat_caster_hint)
    if not escape_dir then
        tlog(3, "grenade_self_skip_unsafe_dest", { push = push_distance })
        return nil
    end
    ...
    -- whenever the helper picks 0deg.
...
state.pick_escape_dir = function(me_pos, toward_threat, push_distance, threat_caster_hint)
    ...
    for _, deg in ipairs({ 0, -35, 35, -65, 65, -90, 90 }) do
        ...
        if SafePushDestination(landing, threat_caster_hint) then
...
state.pike_self_reposition = function(intent, threat_caster, it)
    ...
    -- helper picks the lowest-danger landing among 5 angles off
    -- straight-away (0, -35, 35, -65, 65 degrees), so the chosen
    -- escape_dir IS the new facing target. The user's v6.15.242 TF
    -- report was exactly this case: pike pushing Sniper into a
    -- backliner because the historical code always picked 0deg.
    local PIKE_PUSH = 600
...
local SNIPER_SAVE_OVERRIDES = {
    ...
    -- v6.15.246: prepend Pike and Force Staff. The two original entries
    -- (grenade_self, grenade_at_caster) are both 120-deg facing-gated; the
    -- v6.15.245 fresh-log test showed Sniper back-turned at 142deg inside the
    -- field with both grenade saves rejected and NOTHING else in the chain
    -- to cover. Pike-on-self actively turns Sniper before firing so it has
    -- no facing dependency; Force Staff is 0s self-cast. Both 600u pushes
    -- comfortably clear the 225u field radius. Same pattern as the channel /
    -- charge overrides above (Pike-first across all viable cases).
